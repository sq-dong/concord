...
Request* DpdkLatencyClient::startReq() {
    ...
    while (tsc_to_ns(rte_get_tsc_cycles(), tsc_hz_) < next_gen_ns_)
        rte_pause();
    req->client_id = static_cast<uint16_t>(thread_id_);
...
struct rte_mbuf* DpdkLatencyClient::createPacket(const Request* req) {
    ...
    const uint16_t payload_len = sizeof(Message);
    ...
    msg.pkts_length = sizeof(Message);
    msg.runNs = req->runNs;
    msg.genNs = req->genNs;
    uint8_t* payload = pkt_data + sizeof(struct rte_ether_hdr) +
                      sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_udp_hdr);
    memcpy(payload, &msg, sizeof(Message));
...
bool DpdkLatencyClient::parsePacket(struct rte_mbuf* m, Response* resp) {
    uint16_t min_len = sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) +
                      sizeof(struct rte_udp_hdr) + MESSAGE_WIRE_SIZE;
    ...
    const uint8_t* payload = reinterpret_cast<const uint8_t*>(udp_hdr + 1);
    uint16_t type = *reinterpret_cast<const uint16_t*>(payload + 0);
    if (type != TYPE_RES) {
        return false;
    }

    resp->runNs = read_u64_le(payload + MSG_OFFSET_RUNNS);
    resp->genNs = read_u64_le(payload + MSG_OFFSET_GENNS);
    resp->networker_recv_ns = read_u64_le(payload + MSG_OFFSET_NETWORKER_RECV_NS);
    resp->server_send_ns = read_u64_le(payload + MSG_OFFSET_SERVER_SEND_NS);
    resp->networker_cy = read_u64_le(payload + MSG_OFFSET_NETWORKER_CY);
    resp->dispatcher_cy = read_u64_le(payload + MSG_OFFSET_DISPATCHER_CY);
    return true;
}

void DpdkLatencyClient::finiReqs(const std::vector<Response>& responses) {
    static std::atomic<bool> first_dump{true};
    for (const auto& resp : responses) {
        // if (first_dump.exchange(false)) {
        //     fprintf(stderr, "[client] first response: runNs=%" PRIu64 " genNs=%" PRIu64 " networker_recv_ns=%" PRIu64 " server_send_ns=%" PRIu64 " networker_cy=%" PRIu64 " dispatcher_cy=%" PRIu64 "\n",
        //             resp.runNs, resp.genNs, resp.networker_recv_ns, resp.server_send_ns, resp.networker_cy, resp.dispatcher_cy);
        // }
