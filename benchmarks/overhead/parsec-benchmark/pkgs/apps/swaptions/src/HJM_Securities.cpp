...
#else
	/* No need to do this when using the macro version 
...
*/  
	...
	thrIndex = 0L; 
	for (i = 0; i < (nThreads); i++) {
		Error = pthread_create(&PThreadTable[i], &pthread_custom_attr, (void * (*)(void *))(worker), (void*)thrIndex);
		if (Error != 0) {
...
//	free(threads);
	...
	__parsec_bench_end();
   	...
   	printf("swaptions runtime: %lu usec\n", run_time);
